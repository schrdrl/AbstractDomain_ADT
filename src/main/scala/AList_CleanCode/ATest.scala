...
case class APred(op: String, name: String) extends ATest {
  def positive(value: AVal): Set[AVal] = {
    (op, value) match {
      ...
      case ("isZero", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.split(AInt.zero.lb, "neither").nonEmpty) Set(AInt.zero) else Set()
      case ("isOne", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.split(AInt.one.lb, "neither").nonEmpty) Set(AInt.one) else Set()
      case ("isTop", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f == AInt(None,None)) Set(AInt.top) else Set()
      case ("isNegative", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.===(AInt(None, Some(0)))._1.nonEmpty) f.===(AInt(None, Some(0)))._1.asInstanceOf[Set[AVal]] else Set()
      case ("isPositive", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.===(AInt(Some(0), None))._1.nonEmpty) f.===(AInt(Some(0), None))._1.asInstanceOf[Set[AVal]] else Set()
  ...
  def negative(value: AVal): Set[AVal] = {
    (op, value) match {
      ...
      case ("isZero", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.split(AInt.zero.lb, "neither").nonEmpty) f.split(AInt.zero.lb, "neither").asInstanceOf[Set[AVal]] else Set(f)
      case ("isOne", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.split(AInt.one.lb, "neither").nonEmpty) f.split(AInt.one.lb, "neither").asInstanceOf[Set[AVal]] else Set(f)
      case ("isTop", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f != AInt(None,None)) Set(AInt.top) else Set()
      case ("isNegative", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.===(AInt(None, Some(0)))._2.nonEmpty) f.===(AInt(None, Some(0)))._2.asInstanceOf[Set[AVal]] else Set()
      case ("isPositive", al : AList) =>
        val f = al.flatten_JustAInt().justValue().asInstanceOf[AInt]
        if(f.===(AInt(Some(0), None))._2.nonEmpty) f.===(AInt(Some(0), None))._2.asInstanceOf[Set[AVal]] else Set()
